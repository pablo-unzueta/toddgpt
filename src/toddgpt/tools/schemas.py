...
class AtomicSystem(BaseModel):
    ...
    forces: List[List[float]] = Field(..., description="Forces on each atom as a list of [x, y, z] components")
    symbols: List[str] = Field(..., description="Chemical symbols of the atoms")
    positions: List[List[float]] = Field(..., description="Positions of the atoms as a list of [x, y, z] coordinates")
